...
void print(int q[]) {
   ...
   for(i=0;i<8;i++){
           for(j=0;j<8;j++){
               if(q[j]==i){                                 //when you figure where the Queens is on the array, in column [j] matches with row i, you know
                                                            //that's what place on the board it belongs to, therefore, you switch it to the according bq or wq image
                   if((board[i][j])==&wb) board[i][j]=&bq;  //you must assign the bq when it's wb, and wq when it's bb
                   else board[i][j]=&wq;
               }
           }
       }
